...
void receive_update() {
  ...
  while (true) {
    int size = zmq_recv(actions_receiver, buffer,
                        5, ZMQ_DONTWAIT);
    if (size == -1) break; // if no more messages are available
...
PC (Spezifikation siehe) %TODO add spec to appendix
mit CUDA Unterstützung für durchschnittlich 32 Stunden ausgeführt. Die nachfolgenden Graphen % TODO 32 Stunden stimmt nicht
...
als Input für das Advantage und das Value Netz verwendet. Anschließend wird, wie schon
