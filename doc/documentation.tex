...
\cite[~S.167]{L2018}, also gilt $V_0(a=a_i) = r + V_i$. Soll eine Policy bestimmen werden,
die einen möglichst hohen total Reward erzeugt, muss die Action $a_i$ gefunden werden, die
die maximale Summe aus unmittelbarem und long-term Reward ergibt. Daraus ergibt sich die
Bellman Equation, die in \autoref{aln:bellman_equation} abgebildet ist. Zusätzlich wird der
discount factor $\gamma$ eingeführt, der mit dem long-term Reward multipliziert wird und
dessen Gewichtung mindert.
\begin{align}
...
basieren alle Policy Gradient Verfahren auf der in Formel \ref{aln:PG-Loss} festgelegten Definition,
...
\begin{figure}[htp]
\centering
\includegraphics[scale=0.28]{res/six_levels_results.png}
...
Im Experiment konnte innerhalb von einer Millionen Frames durchgängig kein Reward erreicht
werden, weshalb dieses vorzeitig abgebrochen wurde. Der Grund für das Nichtbestehen könnte
der schwer zu erreichende erste Reward im Level sein. Ein möglicher Lösungsansatz wäre die
Hinzunahme von weiteren Rewards, wodurch der Agent gegebenenfalls besser durch das Level
navigieren könnte. Eine weitere Möglichkeit wäre die Betrachtung anderer Verfahren des
Explorierens. Das Paper \textit{Intrinsically Motivated Reinforcement Learning}\cite{NIPS2004}
von Chentanez, Barto und Singh definiert einen Ansatz der intrinsisch getriebenen Motivation
zum Erforschen des Environments.
...
verwendet wurde, ist im Anhang unter \ref{sec:untrained_level_appendix} dargestellt.
...
beschriebene ''schwerere'' Level nicht durchgespielt werden konnte, lässt sich ableiten,
dass für die Lösung von komplizierteren Environments erneute Anpassungen am Verfahren
...
Bereits während der Umsetzung dieser Arbeit wurde versucht, das entwickelte System auf Rechnern der
Universität auszuführen, um eine Vielzahl weiterer Experimente durchzuführen. Durch die
Echtzeitkomponente des Spiels ist jedoch eine ausreichend leistungsfähige Rechenarchitektur von Nöten,
welche durch die Universitätsrechner nicht gegeben war. Würde das beschriebene Off-Screen Rendering
umgesetzt werden, könnten gegebenenfalls auch Cloud Dienste zur Beschleunigung und Parallelisierung
des Trainingsprozesses verwendet werden. 
