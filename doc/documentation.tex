...
\end{figure}
% TODO Bruno vllt. nochmal darauf eingeben, dass man denken könnte, dass das Env nur immer einen 0.5er Reward schafft, zumindest laut Graphen 
...
Trainingsdaten zu erhalten. Nach Lapan \cite{L2018} tritt ohne diesen Replay Buffer das %TODO Bruno Seite angeben
...
was mit dem CartPole Environment vergleichbar ist. %TODO lass das nochmal disskutieren

Versuche haben gezeigt, dass die Cross-Entropy Methode in ihrer Einsetzbarkeit begrenzt
ist und schwierigere Environments nicht erfolgreich gelöst werden können. %TODO Bruno Quelle
...
der Bellman Equation mit einer benötigten Laufzeit von X Stunden für die Generierung von Y % TODO Philipp
...
Nach Lapan \cite{L2018} speichert der Replay Buffer neben den Samples eine Priorität für % TODO Bruno page needed for citation
jeden Eintrag. Diese Priorität wird nach dem Training proportional zum entstandenen Loss
aktualisiert. Wird ein neues Sample hinzugefügt, so bekommt dieses Sample eine maximale
Priorität, um eingeschätzt werden zu können. Die Wahrscheinlichkeit $P(i)$, dass der i-te % TODO Bruno um eingeschätzt werden zu können? verstehe ich irgendwie nicht
Sample für das Training verwendet wird, berechnet sich aus \autoref{aln:bufprio}.
\begin{align}
...
\cite{wang2015dueling} vorgestellt. Wie auch in Actor-Critic werden die Q-values nicht % TODO Bruno hier würde ich schon den namen von der Person schreiben
...
\autoref{aln:dueling_q_values} gezeigt wird. % TODO Bruno verstehe ich nicht ganz
%TODO 4 advantage werte -> 3, 2, -4, 0, durchschn. = 1 -> von allen abziehen: 2, 1, -5, 0 durchschn. = -1
\begin{align}
...
Samples. % TODO Bruno was ist der max reward, warum wird der reward besser (annahme)?
