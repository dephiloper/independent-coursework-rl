...
maximal zu erwartende Reward der besten Action im State $s'$. Das bedeutet, dass % TODO Bruno, imo ist da ein maximal zu viel (das 2.) ohne das finde ich die aussage schlüssiger oder?
...
Wobei der Hyperparameter $\alpha$ zwischen 0 und 1 gesetzt wird, um den Wert $Q(s, a)$ bei %TODO Bruno: ist das nicht die Learning Rate?
...
% TODO Bruno verschiedene farben erklären
\begin{figure}[htp]
\centering
...
$V(s)$ auswirkt wird ein $step\_size$ Parameter verwendet, der wie in Zuweisung 
\ref{aln:ValueUpdate} benutzt wird.
\begin{align}
...
Gradienten ist hierbei proportional zum value der zu wählenden Action $Q(s,a)$
...
abgehalten werden sich \grqq zu sicher\grqq bei der Berechnung von Actions zu sein 
...
gesampled werden können. Diese Erweiterungsmöglichkeit wird auch als Asynchronous Advantage Actor
Critic oder in Kurzform A3C bezeichnet.
...
vorgeschlagen. Für die Implementierung wurde eine deque verwendet, wie in Listing
\ref{lst:env_observation} gezeigt. Für das Environment resetet, so wird ein neues Bild
aufgenommen und drei mal in den ImageBuffer geschrieben.
...
\begin{lstlisting}[language=C++, caption=Empfangen der Actions,
...
Problemstellung hilfreich, da nicht blockierende abfragen unterstützt werden.\\
...
Teeworlds Quelltext und dessen Umarbeitung war eine Herausforderung.
...
Ein Problem mit der Cross-Entropy Methode ist, dass auf langen Episoden nicht gut gelernt
werden kann, da die Methode nicht nachvollzieht, welche Actions gut und welche Actions
weniger gut funktioniert haben. Eine Episode im Teeworlds Environment gilt als beendet,
wenn in Stacheln gesprungen wurde, wenn das Herz eingesammelt wurde oder wenn ein
Zeitlimit abläuft. Das Zeitlimit wurde für die im weiteren Verlauf beschriebenen
Experimente auf 40 Sekunden gesetzt, was bei einer Framerate 5 Frames pro Sekunde eine
maximale Episodenlänge von 200 Frames bedeutet, was mit dem CartPole Environment
vergleichbar ist.\\
...
Stunden ausgeführt. Die nachfolgenden Graphen % TODO 32 Stunden stimmt nicht
...
\noindent
\newline
Der Trainingsprozess nach dem Einbinden der Erweiterung beträgt 32 Stunden. % TODO wie viele Stunden?
...
ausgewählt werden, was wiederum zu einer überoptimistischen Schätzung zukünftiger values führen
...
%TODO Philipp Bild hinzufügen
%TODO Philipp überprüfen, ob wir mean value of actions haben um zu zeigen das wir weniger
% overestimaten
...
% TODO Bruno: Auswertung Priority Replay Buffer
...
A(s, a)$, der schon bei Actor-Critic behandelt wurde. Jeder Action erhält eine Advantage,
die aussagt, wieviel besser oder schlechter diese Action im Vergleich zu den anderen
Actions ist. Dabei gilt die weitere Einschränkung, dass die Summe der Advantage Werte auf
0 liegen soll. Um das Umsetzen wird der Durchschnitt der Advantage Werte von den Advantage
Werten abgezogen, sodass deren Durchschnitt 0 wird, wie in \ref{aln:dueling_q_values}
gezeigt.
\begin{align}
...
Layern, die wie gewöhnlich initiatisiert werden auch, ebenfalls zwei neue Netze
initialisiert, von denen das erste die Advantage Werte und das zweite den value des States
berechnet. In der \lstinline!forward()!-Funktion wird der Output der Convolutional Layer
...
der neben den anderen vorgestellten Erweiterungen das oben gezeigte DuelingNet verwendet.
Als Vergleichswert ein Agent, dessen Reward in der Abbildung Grau ist, der ebenfalls alle
vorhergehenden Erweiterungen benutzt, jedoch nicht das DuelingNet.
\begin{figure}[htp]
  \centering
...
\subsection{Ergebnisse} % TODO gemeinsam
\label{sec:results}
...
Laufzeit des Spiels durch dieses erzeugt werden und ebenfalls würde dadurch das zyklische Aufnehmen
von Screenshots redundant werden.
