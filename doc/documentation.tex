...
\autoref{sec:six_levels} veranschaulicht. Der Agent konnte abschließend alle sechs Level
...
benutzt wurde. Das Level wird im Anhang unter \ref{sec:hard_level_appendix} gezeigt.

% Abbildung \ref{fig:hard_level_results} zeigt den Trainingsverlauf des Experiments.
...
Um zu testen in wie weit der trainierte Agent abstrahieren kann, wurde er auf einem neuen
für ihn unbekannten Level getestet ohne dieses vorher gelernt zu haben. Das 
...
lieferte. Des Weiteren wurde mit dem Experiment (\ref{sec:untrained_level}) festgestellt, dass
der Agent ein bisher unbekanntes Level (nicht) erfolgreich durchspielen konnte. Daran
lässt sich beurteilen, dass das Verfahren (zwischen unterschiedlichen Situationen
abstrahiert) overfitted und die zu spielenden Levels (nicht) ''auswendig'' lernt.

Aufgrund der Tatsache, dass das in Experiment (\ref{sec:hard_level}) % TODO ref
